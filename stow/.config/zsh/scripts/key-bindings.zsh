...
typeset -gA keys=(
    ...
    Right     '^[[C'
    Left      '^[[D'
    Home      '^[[H' # Fn + Left arrow
    End       '^[[F' # Fn + Right arrow
    Escape    '^['
    Insert    '^[[2~'
    Delete    '^[[3~'
    PageUp    '^[[5~'
...
bindkey "${keys[Ctrl+N]}" insert-last-word
bindkey "${keys[Ctrl+Y]}" accept-and-hold
bindkey "${keys[Ctrl+W]}" backward-kill-word
...
## Vi Bindings
bindkey "${keys[Ctrl+A]}" vi-beginning-of-line
# [Ctrl-e] - move to end of line
bindkey -M viins "${keys[Ctrl+E]}" vi-add-eol
## [Ctrl-f] - move to next word
bindkey -M viins "${keys[Ctrl+F]}" vi-forward-word

...
create_gpg_key() {
    ...
    [ "${FORCE_REINSTALL}" == "true" ] || [ -z "${key_id+x}" ] && {
...
install_brew_deps() {
    ...
    brew up
    brew upgrade
    brew bundle --file ./stow/.config/homebrew/Brewfile || return 0
}
...
install_theme() {
    ...
    [ "${FORCE_REINSTALL}" == "true" ] || [ ! "$(ls -A "${target_dir}")" ] && {
        local temp_dir
        temp_dir="$(mktemp -d)"
        ...
        mkdir -p "${target_dir}"
        rsync -av --exclude='*.md' --exclude='assets' --exclude='license' --remove-source-files "${temp_dir}/${src_dir}"/* "${target_dir}"
...
install_zsh_fast_syntax_highlighting_themes() {
    ...
    zsh -c 'source ${ZDOTDIR:-~/.config/zsh}/.zshrc && fast-theme XDG:catppuccin-mocha 2>/dev/null'
}
...
validate_args() {
    # Ensuring that an exit code of 0, regardless of whether the echo command is executed or not.
